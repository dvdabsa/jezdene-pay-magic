...
export const AdminPanel = () => {
  ...
  const [treasury, setTreasury] = useState<TreasuryInfo>({ balance: 0 });
  ...
  const fetchAdminData = async () => {
    try {
      ...
      // Fetch Treasury (platform) USD account balance (name = 'Treasury')
      const { data: treasuryAccount, error: treasuryErr } = await supabase
      ...
      if (treasuryErr) {
        console.error('Error fetching treasury balance:', treasuryErr);
      }

      setTreasury({ balance: treasuryAccount?.balance ?? 0 });
    } catch (error) {
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Treasury Balance (USD)</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
