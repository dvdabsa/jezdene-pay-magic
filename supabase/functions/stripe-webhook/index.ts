  ...
  try {
    ...
    if (event.type === 'payment_intent.succeeded') {
      ...
      if (!sellerAccountId) {
        ...
        // Record external net inflow from Stripe into Treasury first
        // Net inflow = gross amount received minus Stripe fee
        const netInflow = Math.max(amountTotal - stripeFee, 0);
        if (netInflow > 0) {
          const { error: treasuryCreditErr } = await supabase
            .from('ledger_entries')
            .insert({
          ...
          if (treasuryCreditErr) {
            console.error('Failed crediting Treasury with net inflow:', treasuryCreditErr);
            return new Response('Treasury credit error', { status: 500 });
          }
        }
